 ...
 *
 *   Authorization: Bearer stw_svc_vbs_<secret>
 *
 * VBS holds exactly one such secret - its own. A token issued to another
 * product is not accepted here, which is the point of issuing one per app: a
 * leak from Table cannot provision or read anything in VBS.
 *
...
const EXPECTED_PREFIX = "stw_svc_vbs_";
