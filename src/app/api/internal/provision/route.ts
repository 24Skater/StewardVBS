 ...
 *
 * Called by the Steward console when a church signs up for VBS. It creates this
 * app's tenant root and nothing else — the console owns identity, billing and
 * entitlements, and this app owns its own schema.
 *
 ...
 *    forever, and it is minted by the console.
 * 2. **Idempotent by `orgId`.** The console retries with backoff, so a second
 *    call for a church that already exists must succeed and change nothing.
 *    That is what makes retrying safe rather than merely tolerable.
 *
...
export async function POST(req: NextRequest) {
  ...
  // The unscoped client, because this is the call that brings the tenant into
  // existence: there is no church in context yet to be scoped to.
  const existing = await unscopedPrisma.org.findUnique({
    where: { id: orgId },
  ...
  if (existing) {
    ...
    return NextResponse.json({ state: 'ready', orgId: existing.id, created: false })
  }

  const slugOwner = await unscopedPrisma.org.findUnique({
    where: { slug },
    select: { id: true },
  })

  if (slugOwner) {
    // 409, not 500: a permanent condition the console must not retry into. Its
    // classifier fails fast on any 4xx other than 429, which surfaces the
  ...
  const org = await unscopedPrisma.org.create({
    data: {
      id: orgId,
      slug,
  ...
  return NextResponse.json({ state: 'ready', orgId: org.id, created: true }, { status: 201 })
}
